...
- [ ] **C3 · Safe output filenames** – percent-decode the name taken from the URL, sanitize it, and honor `Content-Disposition`. More relevant since Cycle 4: URLs may now contain `;`, `$`, `(`, `)` and `|`, and the file name is taken from the URL as written, so a download can be saved as `small.txt;v=1` or `a$(cmd).txt`. That is a legal file name, but an awkward one in a shell. Decoding it (`%2f` becomes `/`) must not be able to leave the target directory.
...
- [ ] **F · Tests** – Cycle 0 added characterization tests, Cycle 1 property tests for the stream helper and a `criterion` benchmark (`benches/throttle.rs`), Cycle 2 a state table and property tests for the resume and retry decisions, Cycle 3 property tests for segment planning and resume validation, Cycle 4 property tests for URL validation (it never panics on any input; legal paths and queries are accepted; `..` is refused in every encoding). Known defects still pinned by `known_defect_*` tests, which flip when their item lands: loopback and link-local addresses (C1), percent escapes in file names (C3), upper-case SHA‑256 digests (C6), untrimmed input-file lines (D6). Still to do: a property test for `parse_size`, and a throughput regression gate if CI is added.
...
- [ ] **G · Project hygiene** – remove unused dependencies (`anyhow`, `digest`) and commit `Cargo.lock` (this is a binary crate). The options-struct part is done (`DownloadOptions`), and `regex` was removed in Cycle 4. Existing code is not `rustfmt`-formatted; run `cargo fmt` as its own commit so blame stays readable.
...
1. **Cycle 5 – C1 + C3**: real address classification for the private-host block (flips two `known_defect_*` tests and closes the metadata-address gap), and safe output filenames, which matter more now that URLs may contain shell-significant characters.
2. **Then**: C4 (no-clobber), C2 (an opt-out for private hosts), the remaining low-priority items, and the release cleanup in G (`Cargo.lock`, unused dependencies, `cargo fmt`, a version bump).

...
1. **Cycle 7 – G, the 1.1.0 release**: remove the unused dependencies, track `Cargo.lock`, `cargo fmt`, commit the end-to-end harness, bump the version, and turn `[Unreleased]` into a dated `1.1.0` section. Everything in the high- and medium-priority groups is now done, so this is the natural point to release.
2. **Then**: the low-priority items (D1–D15), in whatever order is most useful. D1 (`--init` throttling new users to 1 MB/s by default) and D2 (a broken config file is silently ignored) are the two most likely to surprise someone.

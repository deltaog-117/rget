...
1. **Cycle 8 – G, the 1.1.0 release**: remove the unused dependencies, track `Cargo.lock`, `cargo fmt`, commit the end-to-end harness, bump the version, and turn `[Unreleased]` into a dated `1.1.0` section. Everything in the high- and medium-priority groups is now done, and Cycle 7 cleared D1–D5, so this is the natural point to release.
2. **Then**: the remaining low-priority items (D6–D15), in whatever order is most useful. D7 (`-j 0` hangs forever) is the smallest fix left and the most likely to surprise someone; D6 (untrimmed `-i` lines) is the one remaining `known_defect_*` test.

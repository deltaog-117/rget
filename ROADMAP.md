...
- [ ] **F · Tests** – Cycle 0 added characterization tests, Cycle 1 property tests for the stream helper and a `criterion` benchmark (`benches/throttle.rs`), Cycle 2 a state table and property tests for the resume and retry decisions, Cycle 3 property tests for segment planning and resume validation, Cycle 4 property tests for URL validation, Cycle 5 a classifier checked against an independent CIDR table over 20,000 addresses, hermetic integration tests for refused redirects and names, and properties showing that no percent-encoding of a URL segment can produce a file name outside the directory. Known defects still pinned by `known_defect_*` tests, which flip when their item lands: upper-case SHA‑256 digests (C6) and untrimmed input-file lines (D6). Still to do: a property test for `parse_size`, and a throughput regression gate if CI is added.
...
1. **Cycle 6 – C4 (+ C6)**: don't silently overwrite an existing file (`--no-clobber`, or an automatic rename), and make the SHA‑256 comparison case-insensitive with a decision on what to do with the file when it does not match.
2. **Then**: the remaining low-priority items (D1–D13), and the release cleanup in G (`Cargo.lock`, unused dependencies, `cargo fmt`, a version bump). With every high-priority item done, a 1.1.0 release is a reasonable point to cut once C4 and G are finished.
